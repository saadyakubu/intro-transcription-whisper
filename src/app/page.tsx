...
const Home = ()=>{
  ...
  const callGetTranscription = async () =>{
    ...
    try {
      ...
      setResponse(data.output.text || `${data.output.error.code} - ${data.output.error.message}` );
    ...
    <main className="flex min-h-screen flex-col items-center justify-between px-24 py-5">
      ...
      <div className="flex h-[35rem] w-[40rem] flex-col items-center bg-gray-600 rounded-xl">
        ...
        <div className="relative w-[80%] bottom-4 flex justify-center">
          <button className="w-max bg-blue-500 px-4 py-2 rounded-sm"
          onClick={callGetTranscription}>Upload</button>
        </div>
